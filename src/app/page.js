...
// ─────────────────────────────────────────────────────────────────────────────
// AUTH PAGE  (src/app/page.js  OR  src/app/login/page.js)
//
// HOW IT WORKS:
//  • New user  → enters Name + 4-digit PIN → account created in Firebase
//  • Returning user → enters Name + PIN → account found & loaded
//  • PIN is stored hashed (simple SHA-256) — no plain text
//  • After login, userId is saved to localStorage so app knows who is logged in
//  • Logout clears localStorage; user can re-login with same Name + PIN
//
// Firebase structure:
//   users/{userId}  →  { name, pinHash, createdAt, sangha (optional) }
// ─────────────────────────────────────────────────────────────────────────────
...
// Generate a stable userId from name (lowercased, stripped)
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!userSnap.exists()) {
        setError(
          'No account found with this name. Please register first or check your spelling.'
        );
        setLoading(false);
        return;
      }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        setError(
          'An account with this name already exists. Please log in instead, or use a slightly different name.'
        );
        setLoading(false);
        return;
      }
      ...
      <div style={styles.card}>
        ...
        <div style={styles.toggleRow}>
          <button
            style={{
              ...styles.toggleBtn,
              background: mode === 'login' ? '#ff6b35' : 'transparent',
          ...
          <button
            style={{
              ...styles.toggleBtn,
              background: mode === 'register' ? '#ff6b35' : 'transparent',
          ...
          <div style={styles.pinDots}>
            {[0, 1, 2, 3].map((i) => (
              <div
                ...
                style={{
                  ...styles.dot,
                  background: pin.length > i ? '#ff6b35' : '#e0d8d0',
                }}
              />
            ))}
          </div>
...
const styles = {
  page: {
    minHeight: '100vh',
    background: 'linear-gradient(135deg, #fdf9f4 0%, #fef3e8 100%)',
    display: 'flex',
    alignItems: 'center',
