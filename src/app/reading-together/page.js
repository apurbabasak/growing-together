...
export default function ReadingTogether() {
  ...
  // myPlans = { bookId: planObject, ... }
  const [myPlans, setMyPlans] = useState({});
  const [saving, setSaving] = useState(false);
  const [view, setView] = useState('home'); // home | selectBook | setup | planDetail | schedule | log | edit
  const [activePlanId, setActivePlanId] = useState(null); // bookId of plan being viewed/edited
    ...
    if (code) {
      ...
      const unsub = onSnapshot(ref, snap => {
        if (snap.exists()) {
          ...
          // support both old single plan and new multiple plans
          ...
          if (memberData.reading_plans) {
            ...
            setMyPlans({ [memberData.reading_plan.bookId]: memberData.reading_plan });
          } else {
            setMyPlans({});
          }
        }
  ...
  const savePlan = async () => {
    ...
    const updatedPlans = { ...myPlans, [selBook.id]: plan };
    await setDoc(ref, {
      ...ex, members: {
        ...ex.members, [userId]: {
          ...ex.members[userId], reading_plans: updatedPlans
        }
      }
    });
    setSaving(false);
  ...
  const deletePlan = async (bookId) => {
    ...
    const memberData = { ...ex.members[userId] };
    memberData.reading_plans = updatedPlans;
    delete memberData.reading_plan; // remove old key if exists
    await setDoc(ref, { ...ex, members: { ...ex.members, [userId]: memberData } });
    setSaving(false);
  ...
  const logReading = async () => {
    ...
    if (!activePlan.schedule?.find(s => s.date === todayStr)) sched.push({ date: todayStr, done: true, from: lf, to: lt, loggedFrom: lf, loggedTo: lt });
    ...
    const updatedPlans = { ...myPlans, [activePlan.bookId]: updatedPlan };
    await setDoc(ref, { ...ex, members: { ...ex.members, [userId]: { ...ex.members[userId], reading_plans: updatedPlans } } });
    setSaving(false);
    setView('home');
  };
  ...
  if (view === 'home') return wrap(<>
    ...
    <div style={{ padding: '0 20px' }}>
      ...
      {/* My Plans */}
      {plansList.length > 0 && <>
        <h2 style={{ color: '#2D2D2D', fontSize: '16px', margin: '0 0 12px' }}>📖 My Reading Plans</h2>
        {plansList.map(plan => {
          const book = BOOKS.find(b => b.id === plan.bookId);
          const doneDays = (plan.schedule || []).filter(s => s.done).length;
          ...
          return (
            <div key={plan.bookId} style={{ ...card, background: 'linear-gradient(135deg,#FFFAF0,#FFF5E0)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                <span style={{ fontSize: '26px' }}>{plan.bookIcon}</span>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: '15px', color: '#2D2D2D', fontWeight: 'bold' }}>{plan.bookTitle}</p>
                  <p style={{ margin: 0, fontSize: '11px', color: '#6B6B6B' }}>{doneDays}/{totalDays} days · {pct}% · Target: {plan.targetDate}</p>
                </div>
                <button onClick={() => deletePlan(plan.bookId)}
                  style={{ background: '#fff5f5', border: '1px solid #fca5a5', borderRadius: '999px', color: '#ef4444', padding: '4px 10px', fontSize: '12px', cursor: 'pointer', fontFamily: 'Georgia, serif' }}>🗑️</button>
              </div>
      ...
      {Object.keys(members).length > 0 && <>
        ...
        {Object.entries(members).map(([uid, m]) => {
          const plans = m.reading_plans
            ? Object.values(m.reading_plans)
            : m.reading_plan ? [m.reading_plan] : [];
  ...
  if (view === 'selectBook') return wrap(<>
    <Header title="Select a Book" sub="Choose a book to create a reading plan" back={() => setView('home')} />
    <div style={{ padding: '0 20px' }}>
      {BOOKS.map(book => {
        const alreadyHasPlan = !!myPlans[book.id];
        return (
          <button key={book.id} onClick={() => initBook(book)}
            style={{ width: '100%', background: alreadyHasPlan ? '#FFFAF0' : 'white', borderRadius: '18px', padding: '16px', marginBottom: '10px', border: alreadyHasPlan ? '2px solid #FFD700' : '1px solid rgba(255,153,51,0.2)', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer', fontFamily: 'Georgia, serif', textAlign: 'left' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {alreadyHasPlan && <p style={{ margin: '3px 0 0', fontSize: '11px', color: '#22c55e' }}>✅ Plan exists — tap to update</p>}
            </div>
  ...
  if (view === 'edit') return wrap(<>
    <Header title={`✏️ Edit: ${selBook?.title}`} sub="Update your target" back={() => setView('home')} />
    <div style={{ padding: '0 20px' }}>
      <div style={card}>
  ...
  // ════ SCHEDULE ════
  if (view === 'schedule' && activePlan) {
    const doneDays = (activePlan.schedule || []).filter(s => s.done).length;
    const totalDays = (activePlan.schedule || []).length;
    const pct = totalDays > 0 ? Math.round((doneDays / totalDays) * 100) : 0;
    return wrap(<>
      <Header title="📅 Daily Schedule" sub={activePlan.bookTitle} back={() => setView('home')} />
      <div style={{ padding: '0 20px' }}>
        <div style={{ ...card, background: 'linear-gradient(135deg,#FFFAF0,#FFF5E0)' }}>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: '#6B6B6B' }}>{doneDays}/{totalDays} days · {pct}% · Target: {activePlan.targetDate}</p>
        </div>
            ...
            <div key={i} style={{ background: day.done ? '#f0fdf4' : isToday ? '#FFFAF0' : 'white', borderRadius: '16px', padding: '14px 16px', marginBottom: '8px', border: day.done ? '1px solid #86efac' : isToday ? '2px solid #FFD700' : '1px solid rgba(255,153,51,0.12)', opacity: isPast && !day.done ? 0.6 : 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: 0, fontSize: '12px', color: '#6B6B6B' }}>{posLabel(activeBook, day.from)} → {posLabel(activeBook, day.to)}</p>
                  {day.done && day.loggedFrom && <p style={{ margin: '3px 0 0', fontSize: '11px', color: '#22c55e' }}>✅ Read: {posLabel(activeBook, day.loggedFrom)} → {posLabel(activeBook, day.loggedTo)}</p>}
                </div>
  ...
  // ════ LOG READING ════
  if (view === 'log' && activePlan) {
    const todaySched = activePlan.schedule?.find(s => s.date === todayStr);
    return wrap(<>
      <Header title="✏️ Log Today's Reading" sub={activePlan.bookTitle} back={() => setView('home')} />
      <div style={{ padding: '0 20px' }}>
        {todaySched && !todaySched.done && (
