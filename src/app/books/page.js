...
const BOOKS = [
  {
    ...
    urlPrefix: 'bg',
    chapters: {
      1:{title:'Observing the Armies',verses:46},
  ...
  {
    id: 'krishna', title: 'Kṛṣṇa, The Supreme Personality', icon: '📓',
...
function getVedabaseUrl(book, cantoNum, chNum, verseNum) {
  if (book.type === 'canto') {
    const lila = book.id === 'cc'
      ? ['adi','madhya','antya'][cantoNum-1]
      : cantoNum;
    return `https://vedabase.io/en/library/${book.urlPrefix}/${lila}/${chNum}/${verseNum}/`;
  }
  return `https://vedabase.io/en/library/${book.urlPrefix}/${chNum}/${verseNum}/`;
}

export default function BooksPage() {
  const [view, setView] = useState('home'); // home | bookDetail | cantoSelect | chapterSelect | verseList | readVerse
  ...
  const cardStyle = { background: 'white', borderRadius: '20px', padding: '18px', marginBottom: '12px', boxShadow: '0 4px 20px rgba(255,153,51,0.1)', border: '1px solid rgba(255,153,51,0.15)' };
  ...
  const fetchVerse = async (book, canto, chapter, verse) => {
    setLoading(true);
    setVerseContent(null);
    try {
      const url = getVedabaseUrl(book, canto, chapter, verse);
      const res = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`);
      const data = await res.json();
      const html = data.contents;

      // Extract Sanskrit
      const sanskritMatch = html.match(/class="[^"]*r-verse[^"]*"[^>]*>([\s\S]*?)<\/div>/);
      const sanskrit = sanskritMatch ? sanskritMatch[1].replace(/<[^>]+>/g,'').trim() : '';

      // Extract synonyms
      const synonymsMatch = html.match(/class="[^"]*r-synonyms[^"]*"[^>]*>([\s\S]*?)<\/div>/);
      const synonyms = synonymsMatch ? synonymsMatch[1].replace(/<[^>]+>/g,'').trim() : '';

      // Extract translation
      const transMatch = html.match(/class="[^"]*r-translation[^"]*"[^>]*>([\s\S]*?)<\/div>/);
      const translation = transMatch ? transMatch[1].replace(/<[^>]+>/g,'').trim() : '';

      // Extract purport
      const purportMatch = html.match(/class="[^"]*r-purport[^"]*"[^>]*>([\s\S]*?)<\/div>/);
      const purport = purportMatch ? purportMatch[1].replace(/<[^>]+>/g,'').replace(/\n+/g,'\n').trim() : '';

      setVerseContent({ sanskrit, synonyms, translation, purport, url });
    } catch(e) {
      setVerseContent({ error: true, url: getVedabaseUrl(book, canto, chapter, verse) });
    }
    setLoading(false);
  };
  ...
  if (view === 'home') return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #FFF8F0, #FFF0E0)', fontFamily: 'Georgia, serif', paddingBottom: '100px' }}>
      ...
      <div style={{ padding: '0 20px' }}>
        <p style={{ color: '#6B6B6B', fontSize: '14px', margin: '0 0 16px', lineHeight: 1.6 }}>
          Browse and read any verse with translation and purport by Srila Prabhupada.
        </p>
        {BOOKS.map(book => (
  ...
  if (view === 'chapterSelect') {
    const chapters = selectedBook.type === 'canto'
      ? selectedBook.cantos[selectedCanto].chapters
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #FFF8F0, #FFF0E0)', fontFamily: 'Georgia, serif', paddingBottom: '100px' }}>
        <PageHeader
          ...
          subtitle="Select Chapter"
          onBack={() => setView(selectedBook.type === 'canto' ? 'cantoSelect' : 'home')}
        />
        <div style={{ padding: '0 20px' }}>
          {Object.entries(chapters).map(([num, ch]) => {
            const chObj = typeof ch === 'object' ? ch : { title: `Chapter ${num}`, verses: ch };
            return (
              <button key={num} onClick={() => { setSelectedChapter(Number(num)); setView('verseList'); }}
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: '14px', color: '#2D2D2D' }}>{chObj.title}</p>
                  <p style={{ margin: '2px 0 0', fontSize: '11px', color: '#6B6B6B' }}>{chObj.verses} verses</p>
                </div>
                <span style={{ color: '#FF9933', fontSize: '18px' }}>›</span>
              </button>
            );
          })}
        </div>
  ...
  if (view === 'verseList') {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #FFF8F0, #FFF0E0)', fontFamily: 'Georgia, serif', paddingBottom: '100px' }}>
        ...
        />
        <div style={{ padding: '0 20px' }}>
          <p style={{ color: '#6B6B6B', fontSize: '13px', margin: '0 0 14px' }}>{verseCount} verses — tap any to read</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {Array.from({ length: verseCount }, (_, i) => i + 1).map(v => (
              <button key={v} onClick={() => {
                setSelectedVerse(v);
                fetchVerse(selectedBook, selectedCanto, selectedChapter, v);
                setView('readVerse');
        ...
        <div style={{ padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <button onClick={() => {
              const chapters = selectedBook.type === 'canto' ? selectedBook.cantos[selectedCanto].chapters : selectedBook.chapters;
              const chObj = chapters[selectedChapter];
              const max = typeof chObj === 'object' ? chObj.verses : chObj;
              ...
              style={{ padding: '10px 20px', borderRadius: '999px', background: '#FFF0E0', border: 'none', color: '#FF9933', fontSize: '14px', cursor: 'pointer', fontFamily: 'Georgia, serif', fontWeight: 'bold' }}>
              Next →
            </button>
          </div>
          ...
          {loading && (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <p style={{ color: '#FF9933', fontSize: '14px' }}>Loading verse...</p>
            </div>
          )}

          {!loading && verseContent && !verseContent.error && (
            <>
              {verseContent.sanskrit && (
                <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #FFFAF0, #FFF5E0)' }}>
                  <p style={{ margin: '0 0 10px', fontSize: '12px', color: '#FF9933', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase' }}>🕉️ Sanskrit</p>
                  <p style={{ margin: 0, fontSize: '16px', color: '#2D2D2D', lineHeight: 1.8, fontStyle: 'italic' }}>{verseContent.sanskrit}</p>
                </div>
              )}
              {verseContent.synonyms && (
                <div style={cardStyle}>
                  <p style={{ margin: '0 0 8px', fontSize: '12px', color: '#FF9933', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase' }}>📖 Word for Word</p>
                  <p style={{ margin: 0, fontSize: '13px', color: '#6B6B6B', lineHeight: 1.7 }}>{verseContent.synonyms}</p>
                </div>
              )}
              {verseContent.translation && (
                <div style={{ ...cardStyle, background: '#f0fdf4', border: '1px solid #86efac' }}>
                  <p style={{ margin: '0 0 8px', fontSize: '12px', color: '#22c55e', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase' }}>✨ Translation</p>
                  <p style={{ margin: 0, fontSize: '15px', color: '#2D2D2D', lineHeight: 1.8, fontWeight: '500' }}>{verseContent.translation}</p>
                </div>
              )}
              {verseContent.purport && (
                <div style={cardStyle}>
                  <p style={{ margin: '0 0 10px', fontSize: '12px', color: '#FF9933', fontWeight: 'bold', letterSpacing: '1px', textTransform: 'uppercase' }}>📜 Purport</p>
                  <p style={{ margin: 0, fontSize: '14px', color: '#2D2D2D', lineHeight: 1.9, whiteSpace: 'pre-line' }}>{verseContent.purport}</p>
                </div>
              )}
              <a href={verseContent.url} target="_blank" rel="noopener noreferrer"
                style={{ display: 'block', textAlign: 'center', padding: '14px', borderRadius: '999px', background: 'linear-gradient(135deg, #FF9933, #FFD700)', color: 'white', textDecoration: 'none', fontSize: '14px', fontFamily: 'Georgia, serif', fontWeight: 'bold', marginTop: '8px' }}>
                🌐 Open on Vedabase.io
              </a>
            </>
          )}

          {!loading && verseContent?.error && (
            <div style={cardStyle}>
              <p style={{ margin: '0 0 12px', fontSize: '15px', color: '#2D2D2D', textAlign: 'center' }}>⚠️ Could not load verse inline.</p>
              <p style={{ margin: '0 0 16px', fontSize: '13px', color: '#6B6B6B', textAlign: 'center' }}>Open it directly on Vedabase:</p>
              <a href={verseContent.url} target="_blank" rel="noopener noreferrer"
                style={{ display: 'block', textAlign: 'center', padding: '14px', borderRadius: '999px', background: 'linear-gradient(135deg, #FF9933, #FFD700)', color: 'white', textDecoration: 'none', fontSize: '14px', fontFamily: 'Georgia, serif', fontWeight: 'bold' }}>
                📖 Read on Vedabase.io
              </a>
            </div>
          )}
